...
##
alternative_trajectories <- function(origin,destination,mode,concentrations_grid,
                               key,output,hour =NULL){
# ------------             Search of trajectories      ---------------- 
# Search for alternative according to tom-tom
  trajectories_tomtom <- function(origin,destination,mode,hour_trajectory=hour){
    df_rbind <- data.frame()
    num_alternative<-5
    ...
    #--- Types de trajectory - ingles-español
    ...
    destination_lat  <- strsplit(destination, ",")[[1]][1]
    destination_long<- strsplit(destination, ",")[[1]][2]
  ...
  trajectory<- trajectories_tomtom(origin,destination,mode = mode, hour_trajectory=hour )
...
"))
    ...
    #  --- Contenido del plot
      ...
      content_less_exp<- paste(sep = "<br/>",
                              ...
                              paste0("<b>PM Concentrations: </b>", less_exposure_route$dailyPM," µg m-3"),
                              paste0("<b>PM exposure: </b>", less_exposure_route$exposure," µg m-3/h"))
      ...
      content_more_exp <- paste(sep = "<br/>",
                              ...
                              paste0("<b>PM Concentrations: </b>", more_exposure_route$dailyPM," µg m-3"),
                              paste0("<b>PM exposure: </b>", more_exposure_route$exposure," µg m-3/h"))
      ...
      # ---  Plot
       map <- leaflet() %>%
        ...
        addAwesomeMarkers(
          ...
          label = "origin") %>%
        addAwesomeMarkers(
          lat = as.numeric(strsplit(destination, ",")[[1]][1]),
          lng = as.numeric(strsplit(destination, ",")[[1]][2]), 
          label = "destination") %>%
        ...
        addPolylines(data = less_polluted_route_line,weight = 2, color ="#08306b",label = "Less polluted route",popup=content_less_cont,group="Less polluted route")%>%
         ...
         addLegend(data = grid,position = "bottomleft", pal = palfac, values = ~grid$category, 
                   title = "PM2.5 Concentrations (µg m-3)")%>%
         # Layers control
       addLayersControl(
...
destination <- "-32.90212,-68.761" # lat- long
...
#
request_df2<-alternative_trajectories (origin,destination,mode,concentrations_grid,
                                    key,output="df",hour)
  
request_plot<-alternative_trajectories (origin,destination,mode,concentrations_grid,
                                       key,output="plot",hour)
request_polyline<-alternative_trajectories (origin,destination,mode,concentrations_grid,
...
prueba_trajectory_tomtom<-trajectories_tomtom(origin,destination,mode,hour_trajectory=hour)
