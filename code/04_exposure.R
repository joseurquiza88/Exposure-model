...
total_exposure <- function (travel_list, mode, concentrations_grid,key,selection,output_exp,
...
"))
  ...
  grid$categories = case_when(grid$dailyPM<=12.1 ~ 'Good',
                                ...
                                grid$dailyPM >35.4 & grid$dailyPM <= 55.4  ~ 'Unhealthy for sensible groups',
  ...
  mapa <- leaflet() %>%
    ...
    addPolylines(data = route_line,stroke = TRUE,opacity = 0.8,weight = c((rbind_df_1$i)+3), color = c(pal(rbind_df_1$i)),group = (as.character(rbind_df_1$route))) %>%
